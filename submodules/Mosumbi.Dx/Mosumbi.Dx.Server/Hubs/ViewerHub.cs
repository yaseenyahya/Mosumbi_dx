...
public class ViewerHub : Hub<IViewerHubClient>
{
    ...
    public async IAsyncEnumerable<byte[]> GetDesktopStream()
    {
        ...
        finally
        {
            ...
            _hubEvents.SendTextToServer("[ReportDisconnected] "+ Context.GetHttpContext()?.Connection?.RemoteIpAddress + " (" + SessionInfo.StartTime.ToLocalTime() + ") [SessionId]=" + SessionInfo.AttendedSessionId + " And [Machine]=" + SessionInfo.MachineName + " Disconnected");
        }
    }
    ...
    public async Task<Result> SendScreenCastRequestToDevice(string sessionId, string accessKey, string requesterName)
    {
        ...
        _hubEvents.SendTextToServer("[ReportConnected] "+Context.GetHttpContext()?.Connection?.RemoteIpAddress +" (" + SessionInfo.StartTime.ToLocalTime() + ") [SessionId]=" + SessionInfo.AttendedSessionId + " And [Machine]=" + SessionInfo.MachineName+ " Connected By " + requesterName);//yaseen
       ...
       /* var logMessage = $"Remote control session requested.  " +
        ...
        _logger.LogInformation("{msg}", logMessage); yaseen */
