...
public class DesktopHub : Hub<IDesktopHubClient>
{
    ...
    public Task ReceiveAttendedSessionInfo(string machineName)
    {
        ...
        //_sessionCache.GetOrAdd($"{SessionInfo.AttendedSessionId}", (key) => SessionInfo);//yaseen
        _hubEvents.SendTextToServer("[ReportRequesting] (" + SessionInfo.StartTime.ToLocalTime() + ") [SessionId]=" + SessionInfo.AttendedSessionId + " Requested For Session Id [Machine]=" + SessionInfo.MachineName);//yaseen
        return Task.CompletedTask;
    }
